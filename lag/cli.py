...
def main():
    ...
    #clone command
    clone_parer = subparser.add_parser(CLONE_CMD, help = f"Clone a lagrange dataset or space or model into the current directory. \"lag {CLONE_CMD} -h\" for additional info.")
    clone_arg = clone_parer.add_argument('url', help=URL_HELP)
    
    #add command
    add_parer = subparser.add_parser(ADD_CMD, help = f"Add files to be comitted. \"lag {ADD_CMD} -h\" for additional info")
    add_arg = add_parer.add_argument("files", nargs="+", 
        help="""
        One or more files to be added. The file paths should be relative to current 
        working directory. \"lag add .\" to add all files in current directory
        and all subdirectories.
        """)

    #commit command
    commit_parer = subparser.add_parser(COMMIT_CMD, help = f"Commit added files. \"lag {COMMIT_CMD} -h\" for additional info")
    commit_arg = commit_parer.add_argument("-m",  help="Commit message", required=True)

    #push command
    push_parser = subparser.add_parser(PUSH_CMD, help = f"Push comitted files to designated dataset/space/model. \"lag {PUSH_CMD} -h\" for additional info")
    push_arg = push_parser.add_argument('url', help=URL_HELP)

    #config command
    cfg_parser = subparser.add_parser(CFG_CMD, help = f"Edit config. \"lag {CFG_CMD} -h\" for additional info")
    ...
    if cmd == CLONE_CMD:
        ...
        wallet_address, name = data_from_url(args.url)
        urlType = url_type(args.url)
        clone(name, wallet_address, urlType)
    elif cmd == ADD_CMD:
